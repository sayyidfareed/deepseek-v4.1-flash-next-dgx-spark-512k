...
class CB3ArenaV2(CB3Arena):
    ...
    def load_prepacked_slot(self, slot: int, planes: dict[str, torch.Tensor], non_blocking: bool = False) -> None:
        """Load one already-CB3-v2 expert without re-quantizing source FP4 weights.

        The public K154 sidecar is produced with this exact v2 layout.  Keep this
        separate from ``load_slot``: silently treating a v1 sidecar as v2 produces
        plausible but incorrect text, which is worse than a startup refusal.
        """
        if not 0 <= slot < self.slots:
            raise ValueError(f"slot {slot} outside arena of {self.slots}")
        fields = (
            ("w1_lo", self.w1_lo, (INTER, DIM // 4)), ("w1_hi", self.w1_hi, (INTER, DIM // 8)),
            ("w1_cb", self.w1_cb, (INTER, 8)), ("s1", self.s1, (INTER, SG1)),
            ("w3_lo", self.w3_lo, (INTER, DIM // 4)), ("w3_hi", self.w3_hi, (INTER, DIM // 8)),
            ("w3_cb", self.w3_cb, (INTER, 8)), ("s3", self.s3, (INTER, SG1)),
            ("w2_lo", self.w2_lo, (DIM, INTER // 4)), ("w2_hi", self.w2_hi, (DIM, INTER // 8)),
            ("w2_cb", self.w2_cb, (DIM, 8)), ("s2", self.s2, (DIM, SG2)),
        )
        if set(planes) != {name for name, _, _ in fields}:
            raise ValueError("prepacked slot has unexpected CB3 plane names")
        for name, target, shape in fields:
            src = planes[name]
            if src.dtype != torch.uint8 or tuple(src.shape) != shape or not src.is_contiguous():
                raise ValueError(f"invalid prepacked {name}: dtype={src.dtype}, shape={tuple(src.shape)}")
            target[slot].copy_(src.to(self.device, non_blocking=non_blocking), non_blocking=non_blocking)
