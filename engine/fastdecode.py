...
# Opt-in parity arm: retain Model.forward's fixed row shapes throughout FastDecoder.
# It is intentionally off until a native-1M cumulative trace proves that the extra
# work is lossless enough to justify its throughput cost.
FAST_REFERENCE_SHAPES = os.environ.get("DSV41_FAST_REFERENCE_SHAPES", "0") == "1"
...
CONTEXT_BUCKET_MIN = max(512, int(os.environ.get("DSV41_CONTEXT_BUCKET_MIN", "32768")))


def indexer_topk_width(index_topk: int, score_rows: int) -> int:
    """How many real score columns an indexer may select before it pads back to ``index_topk``.

    A short native-1M request can deliberately use a 512-position graph bucket.  A ratio-2
    cache then has only 257 score rows, so ``topk(512)`` is invalid.  The reference Model selects
    only the available rows and appends ``-1`` columns afterwards; FastDecoder must preserve that
    fixed-width contract rather than silently expanding the request bucket.
    """
    if index_topk < 1 or score_rows < 0:
        raise ValueError("index_topk must be positive and score_rows nonnegative")
    return min(index_topk, score_rows)


def indexer_score_rows(cache_rows: int, context_cap: int, ratio: int) -> int:
    """Rows visible to an indexer in one static request bucket."""
    if cache_rows < 1 or context_cap < 1 or ratio < 1:
        raise ValueError("cache_rows, context_cap, and ratio must be positive")
    return min(cache_rows, context_cap // ratio + 1)


def context_bucket(required: int, max_seq: int) -> int:
    """Smallest power-of-two decode capacity that covers a complete request.

    The backing KV/index caches remain ``max_seq`` long. This only avoids scoring
    future-zero index rows that the original path immediately masked.
    """
    if required < 1 or max_seq < 1:
        raise ValueError("context limits must be positive")
    if required > max_seq:
        raise ValueError(f"required context {required} exceeds native max_seq {max_seq}")
    cap = max(CONTEXT_BUCKET_MIN, 1 << (required - 1).bit_length())
...
class FastDecoder:
    def __init__(self, model: M.Model, engine, use_graphs: bool = True):
        ...
        self.reference_shapes = FAST_REFERENCE_SHAPES
        a = self.a
        dev = self.dev
        T = T_VERIFY
        src = a.candidate_source_layer
        src_ratio = self.W.layers[src].ratio
        if src_ratio < 1:
            raise ValueError(f"candidate source layer {src} has invalid compression ratio {src_ratio}")
        # Later indexers may mask their scores with the source's candidate table.  That table is
        # only wide enough when they use the same or a more-compressed cache; otherwise a future
        # architecture change would silently slice a too-short mask.
        bad_candidate_consumers = [L for L in self.W.indexers if L > src and self.W.layers[L].ratio < src_ratio]
        if bad_candidate_consumers:
            raise ValueError(
                f"candidate source ratio {src_ratio} is wider than downstream indexer layer(s) "
                f"{bad_candidate_consumers}"
            )
        ...
        self.context_cap = self.c.max_seq
        n_cand = indexer_score_rows(self.c.ik[a.candidate_source_layer].size(0), self.context_cap, src_ratio)
        ...
        # The default fast path deliberately uses plain GEMMs.  The opt-in parity
        # arm keeps the Model.forward row tile so all R.mm/R.hc_mixes reductions
        # use the same shapes during prefill, verify, and DSpark seed/draft.
        R.MM_TILE = M.MM_TILE if self.reference_shapes else 0
    ...
    def set_context_limit(self, required: int) -> int:
        """Select one graph shape for the next request without reducing native capacity."""
        cap = context_bucket(required, self.c.max_seq)
        if cap == self.context_cap:
            return cap
        if self.graphs:
        ...
        src = self.a.candidate_source_layer
        ratio = self.W.layers[src].ratio
        n_cand = indexer_score_rows(self.c.ik[src].size(0), cap, ratio)
        self.candidates = torch.zeros(T_VERIFY, n_cand, dtype=torch.bool, device=self.dev)
        return cap
    ...
    def _hc_mixes(self, x, hc_fn, hc_scale, hc_base):
        if self.reference_shapes:
            return R.hc_mixes(x, hc_fn, hc_scale, hc_base, self.a)
        xb = x.flatten(1)
    ...
    def _attention(self, x, w, L, ring, freqs, pos, sh_state, mtp_last=None):
        ...
        T = x.size(0)
        fq = freqs[pos]
        linear = R.qlinear if self.reference_shapes else _lin
        qr = R.rmsnorm(linear(x, w.wq_a), w.q_norm, a.norm_eps)
        q = self._rope(linear(qr, w.wq_b).view(T, a.n_heads, a.head_dim), fq)
        kv = self._rope(R.rmsnorm(linear(x, w.wkv), w.kv_norm, a.norm_eps), fq)
        ...
        if FUSED_ATTN and not self.reference_shapes:
            o = decode_attention(q, kv1, kv2, mask, w.attn_sink, scale)
        else:
            kv_all = kv1 if kv2 is None else torch.cat([kv1, kv2], dim=1)
            if self.reference_shapes:
                # Model._softmax_attn pads query rows to M.ATTN_TILE and keeps
                # its FP32 PV reduction; all tensor dimensions remain static for
                # a captured verify graph.
                o = self.m._softmax_attn(q, kv_all, mask, w.attn_sink)
            else:
        ...
        o = R.wo_a_proj(o, w.wo_a, tiled=self.reference_shapes)
        return linear(o.flatten(1), w.wo_b)
    ...
    def _compressed(self, x, qr, w, L, pos, st):
        ...
        if w.is_kv_source:
            if r > 1:
                ...
                latent = R.rmsnorm(R.mm(x, w.comp_wkv) if self.reference_shapes else _lin(x, w.comp_wkv),
                                   w.comp_norm, a.norm_eps)
                j0 = pos[0]
            nj = latent.size(0)
            ...
            if L in self.W.indexers:
                ...
                k = R.rmsnorm(R.mm(latent, iw.wk) if self.reference_shapes else _lin(latent, iw.wk),
                              iw.k_norm, a.norm_eps)
    ...
    def _indexer(self, x, qr, L, pos, compress_lens, st):
        if self.reference_shapes:
            return self._reference_indexer(x, qr, L, pos, compress_lens, st)
        a = self.a
        iw = self.W.indexers[L]
        T = x.size(0)
        ...
        # Limit score rows to the selected request bucket. Rows beyond it are future-zero and
        # therefore always removed by the existing ``compress_lens`` mask below.
        n_cap = indexer_score_rows(st["ik"].size(0), self.context_cap, st["ratio"])
        k = st["ik"][:n_cap]
        ...
        k_ = indexer_topk_width(a.index_topk, n_cap)
        ...
        return self.m._pad_topk(idx)

    def _reference_indexer(self, x, qr, L, pos, compress_lens, st):
        """Model._indexer scheduling, bounded to the request's static decode bucket."""
        a = self.a
        iw = self.W.indexers[L]
        T = x.size(0)
        ...
        n_cap = indexer_score_rows(st["ik"].size(0), self.context_cap, st["ratio"])
        n_pad = max(a.index_topk, M.KEY_BLOCK, (n_cap + M.KEY_BLOCK - 1) // M.KEY_BLOCK * M.KEY_BLOCK)
        k = st["ik"][:n_cap]
        if k.size(0) < n_pad:
            k = torch.cat([k, k.new_zeros(n_pad - k.size(0), k.size(1))])
        score = torch.empty(T, n_pad, dtype=torch.float32, device=self.dev)
        for i in range(0, T, M.ATTN_TILE):
            j = min(i + M.ATTN_TILE, T)
            qt, wt = q[i:j], wts[i:j]
            if j - i < M.ATTN_TILE:
                qt = torch.cat([qt, qt.new_zeros(M.ATTN_TILE - (j - i), *qt.shape[1:])])
                wt = torch.cat([wt, wt.new_zeros(M.ATTN_TILE - (j - i), wt.size(1))])
            for jb in range(0, n_pad, M.KEY_BLOCK):
                sc = torch.einsum("thd,nd->thn", qt, k[jb:jb + M.KEY_BLOCK])
                score[i:j, jb:jb + M.KEY_BLOCK] = (sc.float().relu_() * wt[:, :, None]).sum(dim=1)[:j - i]
        cpos = torch.arange(n_pad, device=self.dev)
        score.masked_fill_(cpos[None, :] >= compress_lens[:, None], float("-inf"))
        if L == a.candidate_source_layer:
            candidate = M.Model._select_candidates(score, compress_lens, a.candidate_topk_blocks, a.candidate_block_size)
            self.candidates.copy_(candidate[:, :self.candidates.size(1)])
        elif 0 <= a.candidate_source_layer < L:
            candidate = self.candidates
            if candidate.size(1) < n_pad:
                candidate = F.pad(candidate, (0, n_pad - candidate.size(1)), value=False)
    ...
    def _final(self):
        ...
        if self.reference_shapes:
            self.logits.copy_(R.head_logits(x, self.W.head).float())
        else:
        ...
        m0 = self.W.mtp[0]
        main_x = R.rmsnorm((R.qlinear if self.reference_shapes else _lin)(self.main_hidden.to(torch.bfloat16), m0.main_proj),
                            m0.main_norm, a.norm_eps)
        fq = self.m.freqs_w[self.pos]
        for k, w in enumerate(self.W.mtp):
            kv = self._rope(R.rmsnorm((R.qlinear if self.reference_shapes else _lin)(main_x, w.wkv), w.kv_norm,
                                       a.norm_eps), fq)
    ...
    def _draft(self):
        ...
        logits = (R.head_logits(x, self.W.head) if self.reference_shapes else _lin(x, self.head_bf16).float())
        prev = self.d_tok[0]
        temp = self.d_temp[0]
