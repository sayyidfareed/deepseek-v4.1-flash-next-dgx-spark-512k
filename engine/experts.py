...
class ExpertStore:
    ...
    def warm_start_prepacked_cb3(self, pack_dir: str, manifest: dict, log=print) -> None:
        """Load the K154 CB3-v2 sidecar directly into its resident slots.

        This route intentionally does not call ``read_expert`` or ``load_slot``:
        neither checkpoint FP4 reads nor runtime codebook packing are part of a
        reproducible prepacked serving start.  The manifest's ``slot_order`` is
        also the first tensor axis of each layer shard.
        """
        from engine.prepacked_cb3 import K154, LAYERS, load_layer_cpu, manifest_layer
        expected = LAYERS * K154
        if self.lru_slots < expected:
            raise RuntimeError(f"K154 sidecar needs {expected} LRU slots, arena has {self.lru_slots}")
        if getattr(self.arena, "__class__", type(None)).__name__ != "CB3ArenaV2":
            raise RuntimeError("K154 sidecar requires the CB3-v2 routed-expert arena")
        if self.lru or self.transient_map:
            raise RuntimeError("prepacked warm start requires an empty expert store")
        t0 = time.time()
        loaded = 0
        for layer in range(LAYERS):
            entry = manifest_layer(manifest, layer)
            planes = load_layer_cpu(pack_dir, entry)
            for axis, expert in enumerate(entry["slot_order"]):
                # Physical slots are part of the manifest contract: layer order,
                # then the documented axis-zero order.  Do not use the generic
                # LRU allocator here (it pops from the end of a free list).
                slot = loaded
                self.free_lru.remove(slot)
                self.lru[(layer, expert)] = slot
                self.slot_key[slot] = (layer, expert)
                self.arena.load_prepacked_slot(slot, {name: tensor[axis] for name, tensor in planes.items()})
                loaded += 1
            del planes
            torch.cuda.synchronize()
            log(f"prepacked warm start layer {layer + 1}/{LAYERS}: {loaded}/{expected} experts")
        if loaded != expected or len(self.lru) != expected:
            raise RuntimeError("incomplete prepacked warm start")
        log(f"prepacked warm start done: {loaded} experts resident ({loaded * self.arena.bytes_per_slot / 1e9:.1f} GB) "
            f"in {time.time() - t0:.0f}s; source expert reads={self.stats['loads']}")
