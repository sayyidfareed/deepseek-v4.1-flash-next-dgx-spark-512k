...
PRUNE_RANK_CHOICES = ("sum", "max", "coding")


def _normalised_counts(values, label: str, layer: int) -> list[float]:
    """Normalise one per-layer trace histogram without changing its expert order."""
    total = float(sum(values))
    if total <= 0:
        raise ValueError(f"{label} trace has no routing counts for layer {layer}")
    return [float(v) / total for v in values]


def prune_rank_counts(cc: dict, cg: dict, rank: str) -> dict[int, list[float]]:
    """Build the fixed per-layer pruning score for one supported workload policy.

    ``coding`` deliberately specialises a candidate for a coding-only evaluation.  It is
    not a general-purpose default: it leaves out experts selected only by general prose.
    """
    if rank not in PRUNE_RANK_CHOICES:
        raise ValueError(f"unknown DSV41_PRUNE_RANK {rank!r} (sum | max | coding)")
    if set(cc) != set(cg):
        raise ValueError("coding and general trace layers differ")
    scores = {}
    for L in sorted(cc):
        coding = _normalised_counts(cc[L], "coding", L)
        general = _normalised_counts(cg[L], "general", L)
        if len(coding) != len(general):
            raise ValueError(f"coding and general trace widths differ for layer {L}")
        if rank == "coding":
            scores[L] = coding
        elif rank == "sum":
            scores[L] = [c + g for c, g in zip(coding, general)]
        else:
            scores[L] = [max(c, g) for c, g in zip(coding, general)]
    return scores


def uniform_keep_ids(counts: dict, frac: float) -> dict[int, list[int]]:
    """Top-N expert ids per layer with the accepted deterministic high-id tie rule."""
    import math as _m
    n_keep = max(6, _m.ceil(frac * 384))
    return {int(L): sorted(range(len(c)), key=lambda e: (float(c[e]), e), reverse=True)[:n_keep]
...
def build_keep_masks(counts: dict, frac: float, select: str, device, min_per_layer: int = 24):
    ...
    keep = {}
    if select == "uniform":
        keep = {L: np.asarray(k) for L, k in uniform_keep_ids(counts, frac).items()}
    elif select == "global":
        keys = []
        for L, c in counts.items():
...
class V41Engine:
    ...
    def __init__(self, model_dir: str, max_seq: int = 32768, arena_gb: float | None = None, device: str = "cuda",
                 ...
                 expert_format: str = "fp4", sim_cb2_frac: float = 0.0, expert_pack: str | None = None):
        ...
        self.pack_manifest = None
        if expert_pack:
            # A pack is an immutable model definition, not an optimization hint.
            # Do not accept a trace or a simulated format alongside it: either would
            # make the selected expert set dependent on local state.
            from engine.prepacked_cb3 import K154, load_manifest, verify_release_base
            if self.expert_format != "cb3" or sim_bits or self.sim_cb2_frac:
                raise ValueError("--expert-pack requires --expert-format cb3 without simulation")
            if trace_stats is not None:
                raise ValueError("--expert-pack does not accept --trace-stats; selection is in manifest.json")
            if prune_keep is not None and abs(prune_keep - K154 / 384) > 1e-12:
                raise ValueError("--expert-pack fixes the routed expert count at K154")
            self.pack_manifest = load_manifest(expert_pack)
            # Authenticate the index, selection, and every retained base file
            # before model construction. CB3 sidecars are authenticated exactly
            # once later by warm_start_prepacked_cb3, still before this
            # constructor returns and before the HTTP server can bind.
            verify_release_base(model_dir, expert_pack, self.pack_manifest)
            prune_keep = K154 / 384
            # K154 occupies 6,160 slots and must leave exactly the eight slots the
            # engine's pruned-prefill route needs.  The generic 400-slot default is
            # for streaming models and would silently make this all-resident pack not fit.
            if transient_slots == 400:
                transient_slots = 8
            if transient_slots != 8:
                raise ValueError("--expert-pack requires --transient-slots 8")
        try:
            import fp4_moe as K
        ...
        if host_avail is not None:
            ...
            # A prepacked sidecar copies uint8 planes directly; its bounded
            # layer buffer is still real, but it does not need the GPU FP4→CB3
            # conversion scratch required by runtime packing.
            pack_scratch = 1e9 if self.pack_manifest or self.expert_format == "fp4" else 3e9
        ...
        if self.pack_manifest:
            # No ranking, trace, or local tie-break participates in a packed start.
            # The immutable manifest is both the router allow-list and the arena order.
            masks, ranked = {}, []
            for layer in range(40):
                entry = next(x for x in self.pack_manifest["shards"] if x["layer"] == layer)
                ids = entry["expert_ids"]
                mask = torch.zeros(384, dtype=torch.bool, device=device)
                mask[torch.as_tensor(ids, device=device)] = True
                masks[layer] = mask
                ranked.extend((layer, expert) for expert in entry["slot_order"])
            self.prune_rank = "manifest"
            self.prune_select = "manifest"
            self.model_prune_mask = masks
            log(f"prepacked K154 mode: {len(ranked)} manifest-selected routed experts, "
                f"{len(ranked) * self.expert_bytes / 1e9:.1f} GB")
        elif prune_keep and prune_keep < 1.0:
            ...
            # How the two workloads are combined into one ranking. "sum" (the general-purpose default)
            ...
            # matters to EITHER workload, which is what a general-purpose server needs. "coding"
            # is intentionally narrower: it is only for a separately labelled coding benchmark.
            rank = os.environ.get("DSV41_PRUNE_RANK", "sum")
            counts = prune_rank_counts(cc, cg, rank)
            self.prune_rank = rank
        ...
        if self.pack_manifest:
            self.store.warm_start_prepacked_cb3(self.expert_pack, self.pack_manifest, log=log)
        else:
            self.store.warm_start(ranked, log=log)
        if self.expert_format == "cb3":
            # CB3 warm-start quantizes one source expert at a time. Those temporary tensors are
            # dead after the resident arena is packed, but the caching allocator otherwise retains
            # them and hides several GiB needed by the native-1M serving reserve.
            torch.cuda.empty_cache()
            log("released CB3 warm-start scratch")
        self.fast = None
    ...
    def _decode_loop(self, ids, P, max_tokens, temperature, top_p, stop_ids, out_st, grammar=None, penalties=None):
        ...
        if self.fast is not None:
            # Keep native ``max_seq`` backing caches and positions while graphing only the rows
            # that can contain keys for this request. Future rows are zero and were previously
            # scored only to be masked afterwards.
            self.fast.set_context_limit(P + max_tokens + self._tv)
        if self.swa_replay:
    ...
    def config(self):
        ...
        return {
            ...
            "prune_rank": getattr(self, "prune_rank", None),
            "hot_profile": self.hot_profile,
            "prefill_chunk": MAX_CHUNK,
            "decode_context_cap": getattr(self.fast, "context_cap", None),
            "fast_reference_shapes": getattr(self.fast, "reference_shapes", False),
...
if __name__ == "__main__":
    ...
    ap.add_argument("--expert-pack", default=None,
                    help="directory containing a verified K154 CB3-v2 manifest and 40 layer shards; "
                         "uses its immutable selected-expert list instead of a trace")
    ...
    eng = V41Engine(a.model_dir, max_seq=a.max_seq, arena_gb=a.arena_gb, trace_stats=a.trace_stats,
                    ...
                    sim_cb2_frac=a.sim_cb2_frac, expert_pack=a.expert_pack,
